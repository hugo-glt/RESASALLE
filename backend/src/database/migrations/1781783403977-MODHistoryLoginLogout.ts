...
export class MODHistoryLoginLogout1781783403977 implements MigrationInterface {
    name = 'MODHistoryLoginLogout1781783403977'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`Users\` (\`id_users\` int NOT NULL AUTO_INCREMENT, \`firstname\` varchar(50) NOT NULL, \`password\` varchar(255) NOT NULL, \`email\` varchar(80) NOT NULL, \`id_roles\` varchar(50) NOT NULL, PRIMARY KEY (\`id_users\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`History\` (\`idHistory\` int NOT NULL AUTO_INCREMENT, \`actions\` enum ('CREATE', 'MODIFICATION', 'SUPRESSION', 'CANCEL', 'LOGIN', 'LOGOUT') NOT NULL, \`status\` enum ('CREATE', 'MODIFICATION', 'SUPRESSION', 'CANCEL', 'LOGIN', 'LOGOUT') NOT NULL, \`id_booking\` int NOT NULL, PRIMARY KEY (\`idHistory\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`Booking\` ADD CONSTRAINT \`FK_415b2f0171b8cea2f030ca00da3\` FOREIGN KEY (\`id_users\`) REFERENCES \`Users\`(\`id_users\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
